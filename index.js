...
function kopeginYasi(yas1) {
  /*buraya kodunu yazabilirsin*/
  let köpekYasi = yas1 * 7 ;
  return (yas1 * 7)
}
...
function bilgisayarinSecimi (){
  let secim;
  let tamSayi =Math.floor(Math.random()*3);
  if (tamSayi == 0){
    secim = "Taş"
  }else if (tamSayi == 1 ){
    secim = "Kağıt"
...
for (let maymun=5 ; maymun>0; maymun--){
  console.log(cocukSarkisi(maymun))
}
